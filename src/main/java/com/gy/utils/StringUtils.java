...
public class StringUtils {
	...
	public static String getPk(){
		String currentDate = DateFormatUtils.formatDate(new Date(),
				"yyMMddHHmmss");
		String autoSeq = Math.random()*1000+"";
		String paymentRecordId = "1" + currentDate
				+ appendLeftSub(autoSeq, 4, "0");
	...
	public static String returnStr(String str){
		...
		String encode = MD5Encoder.encode((str+str1+"pajiaug875696._").getBytes());
		return MD5Encoder.encode((encode+"00o_=-+=-0oo00oo0"+str1).getBytes());
	}
	public static Boolean isBlank(String url){
		if(url==null||url.length()==0||url.replace(" ", "").length()==0){
			return true;
		}
		return false;
	}
